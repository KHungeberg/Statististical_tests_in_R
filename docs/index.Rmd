...
In terms of consistency throughout the project, it would be nice with a clean list of check ups i could do so i know that all sections follow the same format. This could be: 

1. Are there any missing sections? (if so; include them even though there might not be much to say) 
2. Is the test statistic and null hypothesis in the same format as the last test? (If not; fix it)
3. Is the code clearly commented and understandable with accurate names?
# Notes on ideas 
...
Maybe join the robustness, interpretations and mininterpretations withing the one section?
...
##### Manual calculations and r-function
```{r}
### Random sample
...
deg_free <- n-1 ## Degrees of freedom
...
p_value <- 2*(1-(pt(t_statistic,df = n-1,lower.tail = F))) # P-value
...
We can visualize the test statistic and p value as such: 

```{r}
t_values <- seq(-4, 4, length.out = 200)


# Calculate the corresponding densities of these t-values
densities <- dt(t_values,deg_free)

# Create the plot
plot_data <- data.frame(t_values, densities)
ggplot(plot_data, aes(x = t_values, y = densities)) +
    geom_line() +
    geom_ribbon(data = subset(plot_data, abs(t_statistic)<=(t_values)),
                aes(ymin = 0, ymax = densities), fill = 'blue', alpha = 0.5) +
  geom_ribbon(data = subset(plot_data, -abs(t_statistic)>=t_values),
                aes(ymin = 0, ymax = densities), fill = 'blue', alpha = 0.5) +
    geom_vline(xintercept = t_statistic, linetype = "dashed", color = "red") +
    geom_vline(xintercept = -t_statistic, linetype = "dashed", color = "red") +
  annotate("text", x = 2.5, y = max(densities), label = paste("T=", round(t_statistic, 2)), hjust = 0) +   
  annotate("text", x = 2.7, y = max(densities)-0.02, label = paste("p-value =", round(p_value, 4)), vjust = 1.5) +
    labs(title = "t-Distribution with Shaded p-value Area",
         x = "t value",
         y = "Density") +
    theme_minimal()
```
...
p_value <- 2*(pt(abs(t_stat),df=deg_f,lower.tail = F)) # P value
...
p_value <- 2*(pt(t_statistic,df = n-1,lower.tail = F)) # P-value
...
## Kruskall - Waliss (maybe not true)

### Purpose

This is the non parametric "equivalent" to the ANOVA test. Here we test whether the centers of several distributions are equal, or rather the mean. 

### Null hypothesis

Suppose we have $k$ groups and sample $x_{ij}$ being observation j of group i. Then our null hypothesis is that all the means are equal. Thus it is the same as for ANOVA. 

$$H_0: \mu_1=\mu_2=\cdots=\mu_k \quad H_A:\mu_a \neq \mu_b \quad \text{for some a and b}$$
### Assumptions

* We assume all samples are drawn from the same population with density $f(x-\mu_i)$. where $f(x)$ is a density with mean 0. 

* Samples are drawn independently 

## Test statistic and distribution

As with the Wilcoxon rank sum and signed rank sum test, we rank the samples from lowest to highest. We also do this here for each group 
...
```{r}
# Assuming previous setup for data and t-test
set.seed(55)
...
# Create a sequence of t-values for plotting
t_values <- seq(-4, 4, length.out = 200)
# Calculate the corresponding densities of these t-values
densities <- dt(t_values, df)

# Create the plot
plot_data <- data.frame(t_values, densities)
ggplot(plot_data, aes(x = t_values, y = densities)) +
    geom_line() +
    geom_ribbon(data = subset(plot_data, abs(t_stat)<=(t_values)),
                aes(ymin = 0, ymax = densities), fill = 'blue', alpha = 0.5) +
  geom_ribbon(data = subset(plot_data, -abs(t_stat)>=t_values),
                aes(ymin = 0, ymax = densities), fill = 'blue', alpha = 0.5) +
    geom_vline(xintercept = t_stat, linetype = "dashed", color = "red") +
    geom_vline(xintercept = -t_stat, linetype = "dashed", color = "red") +
    labs(title = "t-Distribution with Shaded p-value Area (Horizontal Fill)",
         x = "t value",
         y = "Density") +
    theme_minimal()
